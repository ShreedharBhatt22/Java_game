...
public class Game extends JPanel{
  //  JFrame f;
    ...
    public void paint(Graphics g){
        ...
        try{
            obj.drawImage(getToolkit().getImage("images/o.png"), 0, 0 ,this); //draw road on window
            if(cry >= -499 && crx >= -499) //if a road crossing has passed the window view
                obj.drawImage(getToolkit().getImage("images/e.png"),crx,cry,this); //draw another road crossing on window
	...
	//case analysis to restrict car from going outside the right side of the screen
        if(car_x < 0) //if the car has reached or gone over its max x axis value
            car_x = 0; //keep it at its max x axis value
	...
	{  
		     str = "\nCongratulations! "+name+ " Its a high score";  //create a congratulations message	
	}
	else
	{
		     str = "\nBetter Luck Next Time, "+name;  //create a congratulations message			
	}
    ...
    public static void main(String args[]){
        JFrame frame = new JFrame("Car Racing Game Project!");   //creating a new JFrame window to display the game
	//JFrame f= new Frame();
